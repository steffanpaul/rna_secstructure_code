...
numvalid = int((numtrain/0.8)*0.1)
numtest = int((numtrain/0.8)*0.1)
N = numdata
posidx = np.random.permutation(np.arange(N//2))
negidx = np.random.permutation(np.arange(N//2, N))
split_1 = numtrain #int((N//2)*(1-valid_frac-test_frac))
split_2 = numtrain + numvalid #int((N//2)*(1-test_frac))
split_3 = numtrain + numvalid + numtest
...
testidx = np.random.permutation(np.concatenate([posidx[split_2:split_3], negidx[split_2:split_3]]))
...
params_results = '../../results/marks_ts'
if not os.path.isdir(params_results):
    os.mkdir(params_results)


modelarch = 'mlp_%s'%(str(numhidden))
trial = '%s_ts%s'%(exp, str(numtrain))
modelsavename = '%s_%s'%(modelarch, trial)

#img_folder = '%s/Images_mlp'%(exp) #CHANGE
...
'''Som calc'''
if SOMCALC:
  num_summary =  len(test['inputs'])//2
  print ('num_summary:', num_summary)

  arrayspath = 'Arrays/%s.npy'%(modelsavename)
