...
for file in ['resbind_pkhp.py', 'rnn_pkhp.py']:
    for data in ['5', '6']:
        if 'mlp' in file:
            #pretransfer - on hp
        ...
        if 'rnn' in file:
            ...
            #without transferring - on pkhp
            call(['python', file, data, '1', '--justpkhp', '--train', '--test', '--somcalc', '--somvis'])
            call(['python', file, data, '1.3', '--some', '20', '--justpkhp', '--train', '--somcalc', '--somvis'])

            #with transfer
            #call(['python', file, data, '2', '--transfer', '--train', '--test', '--somcalc', '--somvis'])
            call(['python', file, data, '3', '--some', '20', '--transfer', '--train', '--somcalc', '--somvis'])
