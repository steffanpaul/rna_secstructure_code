...
modelarch = 'mlp2layer_%s'%(str(numhidden))
...
if FOM:
  ...
  for ii in num_plots:
      ...
      mf.fom_saliency_mul(X, layer='dense_2_bias', alphabet='rna', nntrainer=nntrainer, sess=sess, ax =ax)
...
if WRITE:
    ...
    if not os.path.isfile('test_metrics.csv'):
        ...
        fd.write('Date,Model Architecture,Hidden Layer Size,Mode Name,Numpos,Loss,Accuracy,AUC-roc,AUC-pr\n')
        fd.close()
    numpos = len(train['inputs'])//2
    metricsline = '%s,%s,%s,%s,%s,%s,%s,%s,%s'%(date, modelarch, numhidden, modename, numpos,
