...
if SOMVIS:
    ...
    plt.figure(figsize=(8,6))
    sb.heatmap(bd.APC_hol(C),vmin=None, cmap='Blues', linewidth=0.0)
    plt.title('Base Pair scores with APC correction: %s %s %s'%(exp, modelarch, trial))

    som_file = modelsavename + 'SoM_bpfilter_APC' + '.png'
    som_file = os.path.join(img_folder, som_file)
    plt.savefig(som_file)
    plt.close()
